...
/* Variablen für Programmalauf */
int Zustand = 0;      //1-Start, 0-Stop
int AlarmAn = 0;      //1-Alarm An, 0-Aus
...
ISR(TIMER0_COMPA_vect){
  ...
  if(milliseconds >= 1000){                                                        //Fehlerbehandlung
    milliseconds = 0;
  } else if (Zustand == 1) {                                                        //bei Zustand Start läuft Timer
    milliseconds ++;  
  }
                                                                                  
  if (Zeit_sec == 0 && Zustand == 1){    //Auslösen Alarm bei Zeit_sec==0, --Zeit_sec bei 1000 Millisekunden
    AlarmAn = 1;
    Zustand = 0;
    Serial.println("AlarmAn 1");
...
void Melodie() {
  if(AlarmAn==1){
    ...
    AlarmAn=0;      //Alarm-Flag wieder auf 0 setzen
  }
}
...
ISR(PCINT1_vect){                                               
 if(~PINC & TA1){            
   ...
   if(Zustand==0 && Zeit_sec < 60*59){
    Zeit_sec += min(60,60-(Zeit_sec%60));       //Rechnet auf höchsten Wert, wenn dieser <60 entfernt liegt
    Serial.print("Zustand:  ");    Serial.println(Zustand);
    Serial.print("Zeit_min: ");    Serial.println(Zeit_sec/60);
   }
 }
 if(~PINC & TA2){            
   Serial.println("Taste2");
   if(Zustand==0 && Zeit_sec > 60){
    Zeit_sec -= max(60,60+(Zeit_sec%60));       //Rechnet auf niedrigsten Wert, wenn dieser <60 entfernt liegt
    Serial.print("Zustand:  ");    Serial.println(Zustand);
    Serial.print("Zeit_min: ");    Serial.println(Zeit_sec/60);
   }
 }
 if(~PINC & TA3){            
   ...
   if(Zustand==1){Zustand=0;}           //Wechsel des Zustandes
   else if(Zustand==0 && !AlarmAn){Zustand=1;};

  if(AlarmAn==1 && Zustand==0){AlarmAn=0;}
   
   Serial.print("Zustand"); Serial.println(Zustand);
   Serial.print("Alarm An"); Serial.println(AlarmAn);
